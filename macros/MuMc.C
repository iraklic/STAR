...
Int_t MinNoMcTpcHits = 10;
enum TrackMcType {kNotDefined, kMatched, kLost, kGhost, kClone, kTotalMcType};
...
const Char_t *proj[5] = {"zx","zy","x","y","yx"};
...
void Init(const Char_t *outfName="Out.root") {
  ...
  for (Int_t s = kPositive; s < kTotalSigns; s++) {
	...
	if (! opt) {
	  ...
	  histsQA[s][k][i]->SetMarkerColor(s+1);   histsQA[s][k][i]->SetLineColor(s+1);  
	  histsK[0][s][k][i]->SetMarkerColor(s+1); histsK[0][s][k][i]->SetLineColor(s+1);
	  histsK[1][s][k][i]->SetMarkerColor(s+1); histsK[1][s][k][i]->SetLineColor(s+1);
	} else {
	  if (k == kGl) {
...
void DrawH3s(TH3F *h3s[2], Int_t animate = 0, Double_t min = 1e9, Double_t max = -1e9, Int_t np = 2) {
  ...
  for (Int_t p = 0; p < np; p++) {// zx, zy, x, y, yx
    for (Int_t s = kPositive; s < kTotalSigns; s++) {
      ...
      if ( p == 0 || p == 2 || p == 4) {h2[s]->GetXaxis()->SetTitle(h3->GetXaxis()->GetTitle()); }
      else                             {h2[s]->GetXaxis()->SetTitle(h3->GetYaxis()->GetTitle()); }
      if ( p == 4)                     {h2[s]->GetYaxis()->SetTitle(h3->GetYaxis()->GetTitle()); }
      h2[s]->SetMarkerColor(h3->GetMarkerColor());
      h2[s]->SetLineColor(h3->GetLineColor());
	...
	if (p < 2 || p == 4) {
	  ...
	  h1[s]->SetLineColor(h2[s]->GetLineColor());
	  h1[s]->GetXaxis()->SetTitle(h2[s]->GetXaxis()->GetTitle());
	} else {
	  h1[s] = (TH1 *) h2[s];
	  h1[s]->GetYaxis()->SetTitle("");
	  h1[s]->SetTitle("");
	}
	...
	if (h1[s]) {
	  ...
	  if (s1[s]) {
	    ...
	    s1[s]->SetLineColor(h2[s]->GetLineColor());
...
//	if (CDir.Contains("devT") && CDir != "devTR") MinNoMcTpcHits = 11;
	cout << "Run with " << CDir.Data() << " set MinNoMcTpcHits = " << MinNoMcTpcHits << " which corrresponds to real hit in TPC" << endl;
	StMuTimer timer;
	Init(outFile);
	timer.start();
...
//							file			// bla.lis read all files in this list, if (file!="") dir is ignored
...
//									10	// maximum number of file to read
	...
	for (Long64_t ev = 0; ev < nevent; ev++)
		...
		if (ev % 100 == 0) cout << "Read event\t" << ev << endl;
		...
		if (Debug())
			{
			for (Int_t i = 1; i <= NoMuMcVertices; i++)
				{
				PrintMcVx(i,MuMcVertices,MuMcTracks);
				}
			}
//		=============  Build map between global and primary tracks from vertex with idTruth == 1
...
//		=============  Build map between global and Mc tracks from primary Mc vertex
	...
	if (mcTrack->No_tpc_hit() >= MinNoMcTpcHits) GiD[3]->Fill(mcTrack->GePid());
      ...
      hists[s][kPr][kMc]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      ...
      default:       l = kMcHit;   break;
      }
      hists[s][kGl][l]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      hists[s][kPr][l]->Fill(TMath::RadToDeg()*mcTrack->Pxyz().phi(), mcTrack->Pxyz().pseudoRapidity(),mcTrack->Pxyz().perp());
      if (l == kMcRec) {
	Int_t Id = mcTrack->Id()-1;
	...
	if (nentries >= 99)
		...
		{
		cout << "--------------------------------" << endl;
		cout << "-- NOT DRAWING ANYTHING !!! ----" << endl;
		cout << "--------------------------------" << endl;
		}
  if (fOut) fOut->Write();
}
