...
void TpcRS(Int_t First, Int_t Last, const Char_t *Run = "y2011,TpcRS", const Char_t *fileIn = 0, const Char_t *opt = "Bichsel", const Char_t *kuip = 0)
	{
...
//	ChainOpt = "MakeEvent,ITTF,ForceGeometry,NoSsdIt,NoSvtIt,Idst,VFMinuit,analysis,dEdxY2,";
	if (! RunOpt.Contains("RC.y",TString::kIgnoreCase) && ! RunOpt.Contains("MC.y",TString::kIgnoreCase))
		{
...
//		ChainOpt += ",tree,";
#if 0
		if (TString(gSystem->Getenv("STAR_VERSION")) == ".DEV2" || TString(gSystem->Getenv("STAR_VERSION")) == "SL11d_embed") ChainOpt += "McTpcAna,NoHistos,NoRunco,noTags,";
		else ChainOpt += "tags,";
#endif
		}
...
//	Bool_t needAlias = kFALSE;
	...
	if (FileIn == "")
		...
		if (RunOpt.Contains("hadr_of",TString::kIgnoreCase) || Opt.Contains("hadr_of",TString::kIgnoreCase)) ChainOpt += "hadr_off,";
		if (RunOpt.Contains("phys_of",TString::kIgnoreCase) || Opt.Contains("phys_of",TString::kIgnoreCase)) ChainOpt += "phys_off,";
		if (RunOpt.Contains("PhysicsOff",TString::kIgnoreCase) || Opt.Contains("PhysicsOff",TString::kIgnoreCase)) ChainOpt += "phys_off,";
		if (! RunOpt.Contains("Y20",TString::kIgnoreCase) && ! RunOpt.Contains("dev",TString::kIgnoreCase)) ChainOpt += "Y2011,";
...
//			geant->Do("gkine 100  14   0.1    10.  -1     1      0    6.28    0.    0.;");
			...
			if ( Opt.Contains("laser",TString::kIgnoreCase))
				...
				if      (Opt.Contains("muon",TString::kIgnoreCase))
					{
					ID =  5;
				...
				else if (Opt.Contains("pion",TString::kIgnoreCase))
					{
					ID =  8;
					mass = 0.1396; 
					if (Opt.Contains("pion-",TString::kIgnoreCase))     ID =  9;
					}
				else if (Opt.Contains("kaon",TString::kIgnoreCase))
					{
					ID = 11;
					mass = 0.4937;
					if (Opt.Contains("kaon-",TString::kIgnoreCase))     ID = 12;
					}
				else if (Opt.Contains("proton",TString::kIgnoreCase))
					{
					ID = 14;
					mass = 0.9383; 
					if (Opt.Contains("antiproton",TString::kIgnoreCase) || Opt.Contains("pbar",TString::kIgnoreCase)) ID = 15;
					}
