...
//void Cosmics(const Char_t *files =  "st_cosmic_adc_19054017_raw_2500014.MuDst.root",
...
//			if (k < 0 || l < 0  ||  Chi2Old >= 1e3) continue;
			if (k < 0 || l < 0  ||  Chi2Old >= 2000) continue; // IRAKLI : allove higher chi2
			Int_t lg = l;
//			Int_t kgc = GlobalTracks_mIndex2Cov[kg];
//			Double_t parK[6] = {CovGlobTrack_mImp[kgc], CovGlobTrack_mZ[kgc], CovGlobTrack_mPsi[kgc], CovGlobTrack_mPti[kgc], CovGlobTrack_mTan[kgc], 0};
//			Double_t covK[15] = {
...
//				CovGlobTrack_mTanImp[kgc], CovGlobTrack_mTanZ[kgc]  , CovGlobTrack_mTanPsi[kgc], CovGlobTrack_mTanPti[kgc], CovGlobTrack_mTanTan[kgc]};
			...
			tracks.chi2 = Chi2Old;
			tracks.runId = MuEvent_mRunInfo_mRunId[0]; 
			tracks.eventId = MuEvent_mEventInfo_mId[0];
			tracks.K = tK;
			tracks.L = tL;
//			Add sector
			...
			tracks.K70 = StDedxPidTraits(kTpcId, kTruncatedMeanId, 100*((Int_t) GlobalTracks_mProbPidTraits_mdEdxTrackLength[kg])+GlobalTracks_mNHitsDedx[kg],
					   GlobalTracks_mProbPidTraits_mdEdxTruncated[kg],GlobalTracks_mProbPidTraits_mdEdxErrorTruncated[kg]);
			tracks.L70 = StDedxPidTraits(kTpcId, kTruncatedMeanId, 100*((Int_t) GlobalTracks_mProbPidTraits_mdEdxTrackLength[lg])+GlobalTracks_mNHitsDedx[lg],
					   GlobalTracks_mProbPidTraits_mdEdxTruncated[lg],GlobalTracks_mProbPidTraits_mdEdxErrorTruncated[lg]);
			tracks.Kfit = StDedxPidTraits(kTpcId, kLikelihoodFitIdentifier, 100*((Int_t) GlobalTracks_mProbPidTraits_mdEdxTrackLength[kg])+GlobalTracks_mNHitsDedx[kg],
					    GlobalTracks_mProbPidTraits_mdEdxFit[kg],GlobalTracks_mProbPidTraits_mdEdxErrorFit[kg]);
			tracks.Lfit = StDedxPidTraits(kTpcId, kLikelihoodFitIdentifier, 100*((Int_t) GlobalTracks_mProbPidTraits_mdEdxTrackLength[lg])+GlobalTracks_mNHitsDedx[lg],
