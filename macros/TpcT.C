...
#endif
#endif
// IRAKLI : making nomper of padrows function of sectors for Y2018
static Int_t NoInnerRows[24] = {0}; // Tpx
static Int_t NoOfRows[24]    = {0};
// \IRAKLI
...
void SetInnerPadrows() {
	for (int i = 0; i < 24; i++) {
		if (NoInnerRows[i] > 0) return;
		TH3 *h3 = (TH3 *) gDirectory->Get("SecRow3");
		if (!h3) {
#ifdef runY2018
			if (i == 19) NoOfRows[i] = 72; // sector 20 (iTPC for Y2018)
			else NoOfRows[i] = 45;
#else
			NoOfRows[i] = 45;
#endif
		}
		else NoOfRows[i] = h3->GetNbinsY();
		NoInnerRows[i] = NoOfRows[i] - 32;
		cout << "SetInnerPadrows: Sector " << i << " : NoInnerRows = " << NoInnerRows[i] << "\tNoOfRows = " << NoOfRows[i] << endl;
...
#ifdef runY2018
	Name_t InOut[4] = {
		{"Inner","Inner for all except 20"},
		{"Outer","Outer for all except 20"},
		{"InnerX","Inner for Sector 20"},
		{"OuterX","Outer for Sector 20"}
	};
#else
	Name_t InOut[2] = {
		{"Inner","Inner TPX"},
...
//			 io  r  pt
	...
	while (iter.Next()) {
		...
		Int_t sector = fPixels_mSector[0]; // IRAKLI : moved this here

		if (! fNoPixels ) {/*cout << "No Pixels in sector " << sector << endl;*/ continue;}
		if (fNoRcHit != 1 || fNoRcTrack < 1) {/*cout << "No Rc Tracks or " << fNoRcHit << " = 1 in sector" << sector << endl; */continue;}
		if (fRcTrack_fNpoints[0] < 10) {/*cout << "points on track less then 10 in sector " << sector << endl;*/ continue;}
...
//    cout << "IRAKLI : sector # " << sector << endl;
...
#endif    
		Int_t io = 0;
		if (row > NoInnerRows[sector]) io = 1;
#ifdef runY2008
		if (sector  == 16) io += 2;
#elif defined runY2018
		if (sector == 20) io += 2; // IRAKLI : FOR RUN 2018 with one iTPC Sector
#endif
...
void TpcTdENP(const Char_t *files="*.root", const Char_t *Out = "") {
  ...
  while (iter.Next()) {
    ...
    if (fNoMcHit != 1 && fNoMcHit != 3) continue;
    for (Int_t k = 0; k < fNoMcHit; k++) {
      int sector = (fMcHit_mVolumeId[k]/100)%100;

      if (fMcHit_mKey[k] != fRcHit_mIdTruth[k]) continue;
      if (fRcHit_mQuality[k] < 95) continue;
      if (fRcHit_mCharge[k] <= 0) continue;
      Int_t io = 0;
      if (fMcHit_mVolumeId[k]%100 > NoInnerRows[sector]) io = 1;
      if (fMcHit_mdE[k] <= 0 || fMcHit_mdE[k] > 1e-3) continue;
      if (fMcHit_mAdc[k] <= 0) continue;
...
//  enum {NPT = 2, NK = 2, IO = 2, NV = 5, NP = 7, NTbPerBin = 3};
  enum {NPT = 2, NK = 2, IO = 4, NV = 5, NP = 7, NTbPerBin = 3}; // IRAKLI
  //           pt   k   i   v   p
...
//  const Char_t *ion[IO]      = {"Inner","Outer"};  // i  
  const Char_t *ion[IO]      = {"Inner","Outer", "Inner20", "Outer20"};  // i // IRAKLI 
...
void FitSlicesT(const Char_t *name="OuterTimeRc", Int_t iy=0, const Char_t *opt="SigmaSqSpread") {
  ...
  if (Name.Contains("X")) prof = (TProfile *) gDirectory->Get("dLInnerX");
  else prof = (TProfile *) gDirectory->Get("dLInner");

  if (Name.Contains("Outer")) {
    lH = lO;
    if (Name.Contains("X")) prof = (TProfile *) gDirectory->Get("dLOuterX");
    else prof = (TProfile *) gDirectory->Get("dLOuter");
  }
  if (! prof) return;
...
//#ifdef runY2018
//	const Char_t *InOut[4] = {"Inner","Outer", "InnerX", "OuterX"};
//	Int_t NS = 4; // TPC + iTPC
//#else
...
//________________________________________________________________________________
void MDFerrorParameterization(const Char_t *treeName = "SpX", Int_t sec = 0, Int_t row = 0, Int_t N = 3, Int_t MS = 1, Int_t prompt = 0, int sector = -1) {
  const Double_t secs[3] = {0.5, 12.5, 24.5};
  const Double_t rows[3] = {0.5, NoInnerRows[sector] + 0.5, NoOfRows[sector] + 0.5};
...
//________________________________________________________________________________
void MDFerrorParameterization2(const Char_t *treeName = "FitP", Int_t iXZ = 0, Int_t sec = 0, Int_t row = 0, Int_t MS = 1, Int_t prompt = 0, int sector = -1) {
#ifdef __TEST__
  //void TpcHitErrors() {
  //                      xz s  r  S  p
 ...
 -0.000328,     0.136,    -0.126,    0.0562,    0.0278,     0.168,   -0.0301,     0.027,    0.0876,     0.025,
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    Int_t r = 0; if (fit.row >  NoInnerRows[sector]) r = 1;
...
//			tree->Show(0);
	...
	TH3D * TSec[2] = {
		new TH3D("TSecI","time bucket difference for Inner sector versus Z by sectors", 210, -210, 210, 800, -2, 2, 24, 0.5, 24.5),
		new TH3D("TSecO","time bucket difference for Inner sector versus Z by sectors", 210, -210, 210, 800, -2, 2, 24, 0.5, 24.5)
	};
...
//	TpcT->Draw("fMcHit.mMcl_t+0.165*Frequency-fRcHit.mMcl_t/64:fMcHit.mPosition.mX3>>TO(210,-210,210,100,-2,3)","fNoMcHit==1&&fNoRcHit==1&&fRcHit.mQuality>90&&fMcHit.mVolumeId%100>13","colz"); TO->FitSlicesY(); TO_1->Fit("pol2","er","",-100,100);
	...
	while (iter.Next()) {
		for (Int_t l = 0; l < fNoRcHit; l++) {
			...
			int sector = (fMcHit_mVolumeId[l]/100)%100;
			Int_t k = -1;
			...
			TSec[io]->Fill(fMcHit_mPosition_mX3[k], dT, sec);
