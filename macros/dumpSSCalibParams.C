...
void dumpSSCalibParams(const char * fileName, const char * part = "") {
	...
	TF1 * fit = new TF1("f", "gaus", -0.2, 0.2);
	...
	for (int i = 1; i <= 24; i++) {
		...
		dXS_zx_py->Fit(fit, "RQ");
		dYS_zx_py->Fit(fit, "RQ");
		dZS_zx_py->Fit(fit, "RQ");
		char nameIt[200];
		sprintf(nameIt, "/afs/rhic.bnl.gov/star/users/iraklic/WWW/Alignment/SuperSector/dXS_fit_%d.gif", i);
		dXS_zx_py->Draw();
		c->SaveAs(nameIt);
		sprintf(nameIt, "/afs/rhic.bnl.gov/star/users/iraklic/WWW/Alignment/SuperSector/dYS_fit_%d.gif", i);
		dYS_zx_py->Draw();
		c->SaveAs(nameIt);
		sprintf(nameIt, "/afs/rhic.bnl.gov/star/users/iraklic/WWW/Alignment/SuperSector/dZS_fit_%d.gif", i);
		dZS_zx_py->Draw();
		c->SaveAs(nameIt);
	}
