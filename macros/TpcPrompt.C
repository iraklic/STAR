/* 
   root.exe -q -b TpcPrompt.C >& TpcPrompt.log & // IRAKLI ; NOT SURE THIS IS NEEDED
   root4star -q -b 'Chain.C+("../*.root","TpcHit")' 'TpcPrompt.C+(tChain)' >& TpcPrompt.log & / IRAKLI : i changed it to work on dev so to access DB now it is run with root4star
